...
import { getPptxJob, getPptxDownloadUrl } from "@/api/pptx";
import PptxJobProgress from "@/components/PptxJobProgress";
...
export default function PptxJobDetail() {
  ...
  useEffect(() => {
    ...
    const fetchJob = async () => {
      try {
        const data = await getPptxJob(jobId);
        setJob(data);
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center gap-4 py-16 text-center">
        ...
        <Link to="/pptx" className="btn-secondary">
          返回创建页
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">PPTX 任务详情</h1>
      ...
      {job.status === "completed" && (
        <div className="flex flex-col items-center gap-4 rounded-2xl bg-green-50 p-8 text-center dark:bg-green-900/20">
          ...
          <p className="text-lg font-semibold text-green-800 dark:text-green-300">PPTX 生成完成！</p>
      ...
      <div className="card dark:border-gray-700 dark:bg-gray-800">
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-gray-500">任务信息</h2>
        <dl className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <dt className="text-gray-500">任务 ID</dt>
            <dd className="mt-0.5 font-mono text-gray-800 dark:text-gray-200">{job.id}</dd>
          </div>
          <div>
            <dt className="text-gray-500">来源类型</dt>
            <dd className="mt-0.5 text-gray-800 dark:text-gray-200">{job.source_type}</dd>
          </div>
          <div>
            <dt className="text-gray-500">风格</dt>
            <dd className="mt-0.5 text-gray-800 dark:text-gray-200">{job.style}</dd>
          </div>
          <div>
